...
func (x *RegisterRsp) FastRead(buf []byte, _type int8, number int32) (offset int, err error) {
	switch number {
	case 1:
		offset, err = x.fastReadField1(buf, _type)
		if err != nil {
...
SkipFieldError:
	return offset, fmt.Errorf("%T cannot parse invalid wire-format data, error: %s", x, err)
ReadFieldError:
	return offset, fmt.Errorf("%T read field %d '%s' error: %s", x, number, fieldIDToName_RegisterRsp[number], err)
}

func (x *RegisterRsp) fastReadField1(buf []byte, _type int8) (offset int, err error) {
...
func (x *RegisterRsp) FastWrite(buf []byte) (offset int) {
	if x == nil {
		return offset
	}
	offset += x.fastWriteField1(buf[offset:])
...
func (x *RegisterRsp) fastWriteField1(buf []byte) (offset int) {
	if x.UId == "" {
		return offset
	}
...
func (x *RegisterRsp) sizeField1() (n int) {
	if x.UId == "" {
		return n
	}
...
var fieldIDToName_RegisterRsp = map[int32]string{
	1: "UId",
}
